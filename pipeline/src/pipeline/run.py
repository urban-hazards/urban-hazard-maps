...
def _process_dataset(dataset: str, raw_records: list[dict[str, Any]]) -> int:
    ...
    page_stats: dict[str, Any] = {
        ...
        "hood_hourly": stats.hood_hourly,
        "zip_hourly": stats.zip_hourly,
        "year_monthly": stats.year_monthly,
        "zip_stats": [z.model_dump() for z in stats.zip_stats],
        "year_hoods": {y: [h.model_dump() for h in hs] for y, hs in stats.year_hoods.items()},
        "year_zips": {y: [z.model_dump() for z in zs] for y, zs in stats.year_zips.items()},
